...
function validateCred(num){
    ...
    for (let i = num.length - 1; i >= 0;i--){
        let number=(i % 2 === 0) ? 2 * num[i] : num[i];
        sum = (number > 9 && i % 2 === 0) ? sum + (number - 9) : sum + number;
    }
    return sum % 10 === 0;
}
...
function idInvalidCardCompanies(invalidArray){
    ...
    invalidComp=[]; // initialized to be an empty array. 
    ...
    return removeDuplicate(invalidComp);
}
